...
exports.weatherApiAllTimeInDay = (req,res) => {
    unirest
        .get('https://api.weatherapi.com/v1/forecast.json?key=dceec52001394e31860192221211403&q=Poznan&days=1&aqi=no&alerts=no')
        .headers({'Accept': 'application/json', 'Content-Type': 'application/json'})
        .then((response) => {
            console.log(response.body.forecast.forecastday[0].hour[0])
            const lastUpdateTemp = getRealdata(response.body.current.last_updated)
